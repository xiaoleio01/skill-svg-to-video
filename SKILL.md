...
**Prompt**：
```
将以下内容拆分成2-10个幻灯片，每个幻灯片是一段简短完整的内容。
要求：
...
**可用风格预设**：
- `bold-editorial` - 新闻编辑风格、专业低调、信任感
...
**重要**：
1. **不将播报内容嵌入图片** - 图片仅作为场景背景，不包含任何文字内容
2. 配图提示词描述内容场景，而非直接嵌入播报文字
3. 新闻类内容使用专业、低饱和度的视觉风格

1. 根据内容类型和风格生成场景契合的英文图片提示词
2. 提示词结构：
   - 内容场景描述（新闻演播室、教室、实验室等）
   - 风格预设的视觉维度（texture, mood, typography）
   - 明确禁止文字叠加

**提示词生成逻辑**：
```
[内容场景描述], [风格基础描述], [texture] texture, [mood] mood, [typography]. 9:16 vertical format, subtle muted tones, no text overlay
```

**新闻类风格要点**：
- 使用 muted tones（低饱和色调）
- 专业、可信赖的视觉感
- 不使用 vibrant/bold 等过于鲜艳的描述词
...
- 如果图像生成失败，再重试一次，重试后还是失败，使用占位 SVG
- 始终先保存成功生成的文件，再报告完成
